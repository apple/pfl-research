...
from mdm.bridge.base import PolyaMixtureFrameworkBridge
...
class FrameworkBridgeFactory:
    ...
    @staticmethod
    def polya_mixture_bridge() -> PolyaMixtureFrameworkBridge:
        framework = get_framework_module().FRAMEWORK_TYPE
        if framework == MLFramework.PYTORCH:
            from .pytorch import polya_mixture as polya_mixture_pt
            return polya_mixture_pt.PyTorchPolyaMixtureBridge
        else:
            raise NotImplementedError("PolyaMixture bridge not available "
                                      f"for framework {framework}")
